...
static void msdc_tasklet_card(unsigned long arg)
{
    ...
    if (hw->get_cd_status) { // NULL
		...
		if(hw->cd_level)
			inserted = (host->sd_cd_polarity == 0) ? 0 : 1;	//huangyisong_add 20130918
		else
        	inserted = (host->sd_cd_polarity == 0) ? 1 : 0;	//huangyisong_add 20130918
    }
...
//#if 1
    if (host->hw->flags & MSDC_CD_PIN_EN) { /* for card, MSDC_CD_PIN_EN set*/
        if(host->hw->cd_level)
			host->card_inserted = (host->sd_cd_polarity == 0) ? 0 : 1; //huangyisong_add 20130918
        else
        	host->card_inserted = (host->sd_cd_polarity == 0) ? 1 : 0; //huangyisong_add 20130918
    } else {
...
static void msdc_enable_cd_irq(struct msdc_host *host, int enable)
{
    ...
    if(enable){
        if(host->id == 1 && (MSDC_SD == hw->host_function) && (hw->flags & MSDC_CD_PIN_EN)){
            ...
            mt_eint_set_hw_debounce(CUST_EINT_MSDC1_INS_NUM, 0);//CUST_EINT_MSDC1_INS_DEBOUNCE_CN //huangyisong_add 20130918
