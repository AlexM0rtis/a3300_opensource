...
extern struct acc_hw* get_cust_bma222e_acc_hw(void);
...
/*huangyisong add for BMA220 direction debug  20120317*/
static ssize_t show_direction(struct device_driver *ddri, char *buf)
{
    ssize_t res;
    struct acc_hw *hw = get_cust_bma222e_acc_hw();
...
/*----------------------------------------------------------------------------*/
static int bma222_gsensor_operate(void* self, uint32_t command, void* buff_in, int size_in,
			...
			else
			{
				...
				sscanf(buff, "%x %x %x", &gsensor_data->values[0], 
					&gsensor_data->values[1], &gsensor_data->values[2]);
                                //gsensor_data->values[1]-=1000;//niyangadd	
                //Huangyisong_add for adjust 20131116
                GSE_LOG("BMA222E SENSOR_GET_DATA X[%d]; Y[%d]; Z[%d]\n",gsensor_data->values[0],gsensor_data->values[1],gsensor_data->values[2]);

				//Y+ axis adjust
                if(gsensor_data->values[1] >=11000)
                    gsensor_data->values[1] = 9840;
                else if(11000> gsensor_data->values[1] &&  gsensor_data->values[1]>10500)
                    gsensor_data->values[1] = 9815;
                else if(10500>= gsensor_data->values[1] &&  gsensor_data->values[1]>10000)
                    gsensor_data->values[1] = 9783;
                else if(10000>= gsensor_data->values[1] &&  gsensor_data->values[1]>9800)
                    gsensor_data->values[1] = 9756;

                //X+ axis adjust
                if(gsensor_data->values[0] >=11000)
                    gsensor_data->values[0] = 9830;
                else if(11000> gsensor_data->values[0] &&  gsensor_data->values[0]>10500)
                    gsensor_data->values[0] = 9815;
                else if(10500>= gsensor_data->values[0] &&  gsensor_data->values[0]>10000)
                    gsensor_data->values[0] = 9773;
                else if(10000>= gsensor_data->values[0] &&  gsensor_data->values[0]>9800)
                    gsensor_data->values[0] = 9736;

                //X- axis adjust
                if(gsensor_data->values[0] <=-11000)
                    gsensor_data->values[0] = -9850;
                else if(-11000< gsensor_data->values[0] &&  gsensor_data->values[0]<-10500)
                    gsensor_data->values[0] = -9815;
                else if(-10500<= gsensor_data->values[0] &&  gsensor_data->values[0]<-10000)
                    gsensor_data->values[0] = -9783;
                else if(-10000<= gsensor_data->values[0] &&  gsensor_data->values[0]<-9800)
                    gsensor_data->values[0] = -9900;//-9756;

                if(-9250>gsensor_data->values[0] && gsensor_data->values[0] >=-9300)
                    gsensor_data->values[0] = -9780;
                else if(-9300> gsensor_data->values[0] &&  gsensor_data->values[0]>-9400)
                    gsensor_data->values[0] = -9795;
                else if(-9400>= gsensor_data->values[0] &&  gsensor_data->values[0]>-9500)
                    gsensor_data->values[0] = -9823;
                else if(-9500>= gsensor_data->values[0] &&  gsensor_data->values[0]>-9700)
                    gsensor_data->values[0] = -9850;
                else if(-9700>= gsensor_data->values[0] &&  gsensor_data->values[0]>-9850)
                    gsensor_data->values[0] = -9880;
#if 1 //huangyisong_delete 20140409
                // x/y/z low value adjust
                if( gsensor_data->values[0]<-500 && gsensor_data->values[0]>-2000)
                    gsensor_data->values[0] = -230;
                else if( gsensor_data->values[0]>0 && gsensor_data->values[0]<2000)
                    gsensor_data->values[0] = 310;

                if( gsensor_data->values[1]<-500 && gsensor_data->values[1]>-2000)
                    gsensor_data->values[1] = -330;
                else if( gsensor_data->values[1]>0 && gsensor_data->values[1]<2000)
                    gsensor_data->values[1] = 280;

                if( gsensor_data->values[2]<-500 && gsensor_data->values[2]>-2000)
                    gsensor_data->values[2] = -260;
                else if( gsensor_data->values[2]>0 && gsensor_data->values[2]<2000)
                    gsensor_data->values[2] = 380;
#endif                   
...
static int bma222_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	obj->hw = get_cust_bma222e_acc_hw();
	...
	sobj.self = obj;
    sobj.polling = 1;
    sobj.sensor_operate = bma222_gsensor_operate;
...
static int __init bma222_init(void)
{
    ...
    if(gsensor_driver_register(&bma222_gsensor_driver))
