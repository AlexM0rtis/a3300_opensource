...
//add by zym for debug battery temperature
int g_battery_test_temperature=0;
int g_battery_test_enable=0;

#if 1//wangliangfeng 20140331 add for identify the flash ID
...
///////////////////////////////////////////////////////////////////////////////////////////
//wangliangfeng 20140402 add for CTA test
#if defined(MTK_CTA_SUPPORT)
extern BOOTMODE g_boot_mode;
kal_bool g_hv_status = KAL_FALSE;
#endif
...
//wangliangfeng
	...
	extern void Enable_Vibrator(void);
	extern void Disable_Vibrator(void);
#endif
kal_bool upmu_is_chr_det(void)
{
    ...
    if(tmp32 == 0)
    {
        //wangliangfeng 20140402 add for CTA test
        #if defined(MTK_CTA_SUPPORT)
        if((battery_meter_get_charger_voltage() > 6800)&&(g_boot_mode == KERNEL_POWER_OFF_CHARGING_BOOT))
		return KAL_TRUE;
		else
		#endif
        return KAL_FALSE;
    ...
    union power_supply_propval *val)
{
    ...
    case POWER_SUPPLY_PROP_CAPACITY:
			//huangyisong_add for runin battery test start
...
{
	battery_xlog_printk(BAT_LOG_CRTI, "BMT_status.UI_SOC = %d\r\n",BMT_status.UI_SOC);
...
unsigned int g_tempStatus = 3;
unsigned int g_tempStatusPre = 6;
//HQ_wangliangfeng 20140418 add
unsigned int g_chargerStatus = 0; //CHARGER_UNKNOWN
unsigned int g_chargerStatusPre = 4; //STANDARD_CHARGER
int set_lenovo_charger_status()
{
	int temperature = 0;
	...
	//wangliangfeng 20140418 add for getting charge type
	g_chargerStatus = BMT_status.charger_type;
    printk("ChargerType: %d, Charger temperature: %d.\r\n",BMT_status.charger_type,temperature);
	...
	{
		if((g_tempStatus == 2) || ((g_tempStatus == 1) && (temperature >= 2)) || ((g_tempStatus == 3) && (temperature <= 8)))
		{
			g_tempStatus = 2;
			if((g_tempStatus == 1) || (g_tempStatus == 2))//recovery from under 0,send notefication
			{
				g_BatteryNotifyCode |= 0x1000;
			}
			else
	...
	else if(temperature > 10 && temperature <= 45)
	{
		if(((g_tempStatus == 2) && (temperature >= 11)) || ((g_tempStatus == 4) && (temperature <= 43)))
		{
			g_tempStatus = 3;
		}
		g_BatteryNotifyCode &= ~(0x1000);
	...
	{
		if((g_tempStatus == 4) || ((g_tempStatus == 3) && (temperature >= 46)) || ((g_tempStatus == 5) && (temperature <= 48)))
		{
			g_tempStatus = 4;
			if((g_tempStatus == 5) || (g_tempStatus == 4))//recovery send notefication
			{
				g_BatteryNotifyCode |= 0x1000;
			}
			else
    ...
    //wangliangfeng 20140418 add
	if(g_tempStatusPre == g_tempStatus && g_chargerStatusPre == g_chargerStatus)
		return 0;//no change,return

	g_tempStatusPre = g_tempStatus;
	g_chargerStatusPre = g_chargerStatus; //wangliangfeng 20140418 add
	//handle
	switch(g_tempStatus)
	{   
		case 0:break;
		...
		case 3:
			{	
				if (BMT_status.charger_type == STANDARD_CHARGER) 
					set_bat_charging_current_limit(1200);
			   	else				
					set_bat_charging_current_limit(450);
				break;
			}
...
static PMU_STATUS mt_battery_CheckChargerVoltage(void)
{
	...
	#if 1//defined(MTK_CTA_SUPPORT) //wangliangfeng 20140402 add for CTA test
	else
	BMT_status.bat_charging_state = CHR_PRE;
	#endif	            
    }
...
#endif
#endif
	//add by zym for less min_charge_temperature
	if(BMT_status.temperature <= MIN_CHARGE_TEMPERATURE) 
    {
        g_BatteryNotifyCode |= 0x0100;
        battery_xlog_printk(BAT_LOG_CRTI, "[BATTERY] bat_temp(%d) less of range\n", BMT_status.temperature);
...
{
battery_xlog_printk(BAT_LOG_CRTI, "[CTA mt_battery_notify_VCharger_check] charger_vol:%d; V_CHARGER_MAX:%d\n",BMT_status.charger_vol,V_CHARGER_MAX); 
#if defined(BATTERY_NOTIFY_CASE_0001_VCHARGER)
	if(BMT_status.charger_vol > V_CHARGER_MAX)
    {   
	    //wangliangfeng 20140402 add for CTA test
	    #if defined(MTK_CTA_SUPPORT)
        if(g_hv_status == KAL_FALSE)
    	g_BatteryNotifyCode |= 0x0020;
		#else
...
#if !defined(MTK_CTA_SUPPORT)//wangliangfeng 20140402 add        
...
void BAT_thread(void)
{
	...
	if(upmu_is_chr_det() == KAL_TRUE)
		set_lenovo_charger_status();//add by zym
	
	//HQ_wangliangfeng 20130906 add for vibration when press powerkey until lighting backlight in usb mode start
    #ifdef HQ_BOOT_WITH_VIBRATOR
		#ifdef MTK_KERNEL_POWER_OFF_CHARGING
		...
		#endif
	#endif 
    //HQ_wangliangfeng 20130906 add for vibration end.
    if( BMT_status.charger_exist == KAL_TRUE )
    {
...
static long adc_cali_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	//add by zym for RunIn interface
    ...
    switch(cmd)
    {
		...
		//huangyisong_add_20130312 start
		//3->On  ;  2->Off
		...
		{
            battery_xlog_printk(BAT_LOG_CRTI, "[BATTERY] adc_cali_ioctl cmd is 1 and turn on chg!!\n\r");  
			g_runin_test_enter = 0;
            g_runin_battery_test_flag =1;
			pchr_turn_on_charging();
		}
		//huangyisong_add_20130312 end
		...
		case Get_META_BAT_VOL:
            ...
            xlog_printk(ANDROID_LOG_DEBUG, "Power/Battery", "****g_emmc_raw_cid 00 :0x%x\n", g_emmc_raw_cid[0]);   
            xlog_printk(ANDROID_LOG_DEBUG, "Power/Battery", "****g_emmc_raw_cid 01 :0x%x\n", g_emmc_raw_cid[1]);   
            xlog_printk(ANDROID_LOG_DEBUG, "Power/Battery", "****g_emmc_raw_cid 02 :0x%x\n", g_emmc_raw_cid[2]);   
            xlog_printk(ANDROID_LOG_DEBUG, "Power/Battery", "****g_emmc_raw_cid 03 :0x%x\n", g_emmc_raw_cid[3]);   
			temp_buf[0] = (g_emmc_raw_cid[0] >> 24) & 0xFF; /* Manufacturer ID */
            ...
            xlog_printk(ANDROID_LOG_DEBUG, "Power/Battery", "****g_emmc_raw_cid 01 after :0x%x\n", temp_buf[0]);   
            xlog_printk(ANDROID_LOG_DEBUG, "Power/Battery", "****g_emmc_raw_cid 02 after :0x%x\n", temp_buf[1]);   
            xlog_printk(ANDROID_LOG_DEBUG, "Power/Battery", "****g_emmc_raw_cid 03 after :0x%x\n", temp_buf[2]);  
            xlog_printk(ANDROID_LOG_DEBUG, "Power/Battery", "****g_emmc_raw_cid 04 after :0x%x\n", temp_buf[3]);   
            xlog_printk(ANDROID_LOG_DEBUG, "Power/Battery", "****g_emmc_raw_cid 05 after :0x%x\n", temp_buf[4]);   
            xlog_printk(ANDROID_LOG_DEBUG, "Power/Battery", "****g_emmc_raw_cid 06 after :0x%x\n", temp_buf[5]);    
            xlog_printk(ANDROID_LOG_DEBUG, "Power/Battery", "****g_emmc_raw_cid 07 after :0x%x\n", temp_buf[6]);  
            xlog_printk(ANDROID_LOG_DEBUG, "Power/Battery", "****g_emmc_raw_cid 08 after :0x%x\n", temp_buf[7]);   
            xlog_printk(ANDROID_LOG_DEBUG, "Power/Battery", "****g_emmc_raw_cid 09 after :0x%x\n", temp_buf[8]); 
            xlog_printk(ANDROID_LOG_DEBUG, "Power/Battery", "****g_emmc_raw_cid 10 after :0x%x\n", temp_buf[4]<<8);           
            xlog_printk(ANDROID_LOG_DEBUG, "Power/Battery", "****g_emmc_raw_cid 11 after :0x%x\n", temp_buf[4]<<8 | temp_buf[5]);                  
			adc_out_data[1] = (temp_buf[4]<<8 | temp_buf[5]);
        	xlog_printk(ANDROID_LOG_DEBUG, "Power/Battery", "****g_emmc_raw_cid adc_out_data[1] :0x%x\n", adc_out_data[1]);   
			#endif
		...
		case Get_META_BAT_SOC:
			user_data_addr = (int *)arg;
            ret = copy_from_user(adc_in_data, user_data_addr, 8);
			adc_out_data[0] = BMT_status.UI_SOC - 5;//add by zym -5
			ret = copy_to_user(user_data_addr, adc_out_data, 8); 
...
int charger_hv_detect_sw_thread_handler(void *unused)
{
    ...
    #if defined(MTK_CTA_SUPPORT)//wangliangfeng 20140402 add for CTA test
        g_hv_status = hv_status;
    #endif
...
static void battery_shutdown(struct platform_device *dev)    
{
	...
	//add by zym 
	if(BMT_status.charger_type == STANDARD_CHARGER)
		set_rtc_spare_fg_value(BMT_status.UI_SOC + 100);
