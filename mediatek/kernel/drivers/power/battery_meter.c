...
void dod_init(void)
{
	...
	//add by zym for power off with ac adapter
	if(g_rtc_fg_soc > 100 )
	{
		gFG_capacity_by_v = g_rtc_fg_soc - 100;
		g_rtc_fg_soc  = gFG_capacity_by_v;  
	}

	if(g_rtc_fg_soc == 0 && gFG_capacity_by_v < 5)
		g_rtc_fg_soc = 1;
	//add by zym
...
kal_int32 mtk_imp_tracking(kal_int32 ori_voltage, kal_int32 ori_current, kal_int32 recursion_time)
{
	...
	#if 1 //defined(A815_SUPPORT_FEATURE)//huangshiguo 131025 add for not match between shutting down & booting
    if(ori_voltage < 3550)
...
void oam_init(void)
{
    ...
    //add by zym
	bm_print(BM_LOG_CRTI,"+++zym_before+++ gFG_voltage = %d,vol_bat = %d\n",gFG_voltage,vol_bat);
	if(vol_bat < 4000)
	{
		if(BMT_status.charger_type == STANDARD_CHARGER)
		{
			vol_bat -= 95;
			bm_print(BM_LOG_CRTI,"+++standard_charger zym_next+++ gFG_voltage = %d,vol_bat = %d\n",gFG_voltage,vol_bat);

		}
		else
		{
			vol_bat -= 65;
			bm_print(BM_LOG_CRTI,"+++nonstandard_charger zym_next+++ gFG_voltage = %d,vol_bat = %d\n",gFG_voltage,vol_bat);
		}
