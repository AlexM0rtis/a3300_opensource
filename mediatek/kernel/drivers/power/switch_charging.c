...
#define FULL_CHECK_TIMES		6
 ...
 CHR_CURRENT_ENUM g_temp_CC_value_storage = CHARGE_CURRENT_0_00_MA;
...
{
	printk("BMT_status.charger_type =%d, g_temp_input_CC_value = %d,g_temp_CC_value = %d.\r\n",
		BMT_status.charger_type,g_temp_input_CC_value,g_temp_CC_value);
    ...
    { 
        ...
        g_temp_input_CC_value = g_temp_CC_value_storage;
        g_temp_CC_value = g_temp_CC_value_storage;
...
void pchr_turn_on_charging (void); //wangliangfeng 20140326 modified
kal_uint32 set_bat_charging_current_limit(int current_limit)
{
    battery_xlog_printk(BAT_LOG_FULL, "[BATTERY] set_bat_charging_current_limit (%d)\r\n", current_limit);
	...
	g_temp_CC_value_storage=g_temp_CC_value;
...
extern unsigned int g_tempStatus;
...
void pchr_turn_on_charging (void)
{
    ...
    else
    {
		...
		printk("pchr_turn_on_charging: g_bcct_flag = %d.\r\n",g_bcct_flag);
        if (g_bcct_flag == 1)
        {
    ...
    //huangyisong_add_20130926 start
...
void mt_battery_charging_algorithm()
{
    ...
    //add by zym for sync aw800 RunIN
